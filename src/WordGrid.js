...
const WordGrid = ({ firstBoard }) => {
  ...
  const tWords = new Map(
    Object.entries(board.groups).map((f) => [
      f[0],
      board.words.filter((w) => w[0].group == f[0])[0].map((f) => f.text),
    ]),
  );
  const words = board.words.flatMap((f) => f.map((e) => e.text));
  const [yellowTargetWords, setYellowTargetWords] = useState(
    tWords.get("yellow").map((w) => w.toLowerCase()),
  );
  const [greenTargetWords, setGreenTargetWords] = useState(
    tWords.get("green").map((w) => w.toLowerCase()),
  );
  const [blueTargetWords, setBlueTargetWords] = useState(
    tWords.get("blue").map((w) => w.toLowerCase()),
  );
  const [purpleTargetWords, setPurpleTargetWords] = useState(
    tWords.get("purple").map((w) => w.toLowerCase()),
  );
  ...
  useEffect(() => {
    ...
    if (
      yellowTargetWords.length > 0 &&
      yellowTargetWords.every((target) => selectedWords.includes(target))
    ) {
    ...
    if (
      greenTargetWords.length > 0 &&
      greenTargetWords.every((target) => selectedWords.includes(target))
    ) {
      const updatedGrid = [...grid];
      updatedGrid.forEach((row, rowIdx) => {
        row.forEach((cell, cellIdx) => {
    ...
    if (
      blueTargetWords.length > 0 &&
      blueTargetWords.every((target) => selectedWords.includes(target))
    ) {
      const updatedGrid = [...grid];
      updatedGrid.forEach((row, rowIdx) => {
        row.forEach((cell, cellIdx) => {
    ...
    if (
      purpleTargetWords.length > 0 &&
      purpleTargetWords.every((target) => selectedWords.includes(target))
    ) {
      const updatedGrid = [...grid];
      updatedGrid.forEach((row, rowIdx) => {
        row.forEach((cell, cellIdx) => {
  ...
  const handleWordClick = (rowIndex, columnIndex) => {
    ...
    if (clickedWord.selected) {
      ...
      const selectedCount =
        updatedGrid.flat().filter((cell) => cell.selected && !cell.locked)
          .length + 1;
      // if (selectedCount <= 4) {
      // Select the word
            ...
            >
              {cell.word.split(" ").map((wordPart, index) => (
                <span key={index}>
                  ...
                  {/* Add newline for words with spaces */}
                </span>
              ))}
            </div>
          ))}
